...
io.on('connection', (socket) => {
    ...
    // setInterval(() => {//keep sending data to client every 2sec
    //     //this message is present agnst the key message which is also same on client side in script.js
    //     console.log("User disconnected with id: ", socket.id);
    //     socket.emit("message", "message from server" + "-" + socket.id + "at" + new Date());
    // }, 2000);
    // // Disconnect event is fired when a user disconnects from the server.by closing tab
